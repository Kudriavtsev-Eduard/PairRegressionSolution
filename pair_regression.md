...
$r_{x, y}=\dfrac{1/n \cdot \sum(y_i-y_{avg})(x_i-x_{avg})}{\sqrt{1/n \cdot \sum(x_i-x_{avg})^2} \cdot \sqrt{1/n \cdot \sum(y_i-y_{avg})^2}}$
...
Статистика: $t_н = \dfrac{|r| \cdot \sqrt{n-2}}{\sqrt{1-r^2}}$ При верной нулевой гипотезе имеет распределение Стьюдента с n-2 степенями свободы.
...
Статистика $t_н=\dfrac{\hat{\theta_i}}{S_{\theta_i}}$, где $S_{\theta_i}$ - стандартная ошибка коэффициента, $S_a = \dfrac{s  \cdot  \sqrt{\sum x_i^2}}{\sqrt{n \sum (x_i - x_{avg})^2}}$, $S_b = \dfrac{s}{\sqrt{n \sum (x_i - x_{avg})^2}}$, $s^2=\dfrac{ \sum \epsilon_i^2 }{n - 2}$, при верной нулевой гипотезе имеет распределение Стьюдента с n-2 степенями свободы.
...
$A_{avg}=\dfrac{1}{n} \sum |\dfrac{y_i-\hat{y_i}}{y_i}| \cdot 100$%

Показывает, на сколько % в среднем модель ошибается в расчетах

### Средняя эластичность

$E_{avg} = b \cdot \dfrac{x_{avg}}{y_{avg}}$
...
1. Строится первичная регрессия, находится ряд остатков
2. Строится регрессия $\epsilon_i^2$ на константу, $x_i$, $x_i^2$ $x_i \cdot x_j$
