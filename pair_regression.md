...
$th( arcth(r^{*}) - \dfrac{u_{\gamma}}{\sqrt{n-3}} ) < r_{x, y} < th( arcth(r^{*}) + \dfrac{u_{\gamma}}{\sqrt{n-3}} )$, где $u_{\gamma}$ - квантиль стандартного нормального распределения уровня $1-\dfrac{\alpha}{2}$
...
Статистика критерия: $F_н=\dfrac{D_{факт}}{D_{остат}}=\dfrac{RSS/m}{ESS/(n-m-1)}$, где m - количество факторов$(m=1)$. $RSS= \sum (y_i^{*}-y_{avg})^2$ $ESS= \sum (y_i - y_i^{*})^2$, при верной нулевой гипотезе имеет распределение Фишера-Снедекора со степенями свободы $m$ и $n-m-1$
...
Статистика $t_н=\dfrac{\theta_i^{*}}{S_{\theta_i}}$, где $S_{\theta_i}$ - стандартная ошибка коэффициента, $S_a = \dfrac{s * \sqrt{\sum x_i^2}}{\sqrt{n \sum (x_i - x_{avg})^2}}$, $S_b = \dfrac{s}{\sqrt{n \sum (x_i - x_{avg})^2}}$, $s^2=\dfrac{ \sum \epsilon_i^2 }{n - 2}$, при верной нулевой гипотезе имеет распределение Стьюдента с n-2 степенями свободы.
...
$A_{avg}=\dfrac{1}{n} \sum |\dfrac{y_i-y_i^{*}}{y_i}|*100$%
